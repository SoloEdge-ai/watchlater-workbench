(function (root, factory) {
  ...
  function resolveContentType(item) {
    ...
    const hasXVideoThumbnail = mediaUrls.some((url) => /pbs\.twimg\.com\/(?:amplify_video_thumb|ext_tw_video_thumb)\//i.test(String(url || "")));
    if (item?.platform === "bilibili" || item?.platform === "youtube" || item?.kind === "video" || item?.hasVideo === true || hasXVideoThumbnail) return "video";
    return "post";
  }
  ...
  function classifyVideo(item, rules = DEFAULT_RULES) {
    const haystack = `${item.title || ""} ${item.bodyText || ""} ${item.quotedText || ""} ${item.creator || item.author || ""} ${item.creatorHandle || ""}`.toLocaleLowerCase();
  ...
  function mergeVideoRecord(existing, incoming, now = Date.now()) {
    const protectedFields = ["rating", "manualCategory", "manualTags", "manualArchived", "aiCategory", "aiTags", "aiConfidence", "sourceRemovalState", "sourceRemovalError", "sourceRemovedAt", "sourceRestoreState", "sourceRestoreError", "sourceRestoredAt"];
  ...
  function enrichVideo(item, rules = DEFAULT_RULES, now = Date.now()) {
    ...
    return { ...classified, contentType: resolveContentType(classified), priorityScore: computePriorityScore(classified, weights, now) };
  }
  ...
  return { DEFAULT_RULES, clamp, clean, sanitizeRules, classifyVideo, mergeVideoRecord, enrichVideo, resolveContentType, computePriorityScore, createSerialQueue, parseAiClassificationResponse };
});
