(function (root, factory) {
  ...
  const { sanitizeRules, mergeVideoRecord, enrichVideo, resolveContentType, clean } = Core;
  const SYNC_URLS = {
  ...
  async function handleMessage(message, sender) {
    ...
    switch (message?.type) {
      ...
      case "START_SOURCE_RESTORE": assertExtensionSender(sender); return getSourceActionCoordinator().start(message.id, "restore");
  ...
  async function completeSourceSync(platform, sessionId, seenIds, tabId, allowEmptySnapshot = false, rawAccount) {
    return withWriteLock(async () => {
      ...
      if (!ids.length && !allowEmptySnapshot) throw new Error("同步结果为空，未执行归档");
  ...
  async function getLibrary(query) {
    ...
    const filtered = all.filter((item) => {
      ...
      if (query.contentType && query.contentType !== "all" && resolveContentType(item) !== query.contentType) return false;
      ...
      if (query.duration && query.duration !== "all" && (item.kind || "video") !== "video") return false;
      ...
      if (search && !`${item.title || ""} ${item.bodyText || ""} ${item.quotedText || ""} ${item.quotedCreator || ""} ${item.creator || ""} ${item.creatorHandle || ""} ${item.category || ""} ${(item.tags || []).join(" ")}`.toLocaleLowerCase().includes(search)) return false;
      return true;
    });
    ...
    const items = filtered.slice(offset, offset + limit).map((item) => {
      const base = item.platform === "bilibili" ? "https://www.bilibili.com/" : item.platform === "x" ? "https://x.com/" : "https://www.youtube.com/";
      const thumbnailUrl = Collectors.absoluteUrl(item.thumbnailUrl, base);
      const compatible = { ...item, kind: item.kind || "video", contentType: resolveContentType(item), sourceItemId: item.sourceItemId || item.videoId || item.id?.split(":").slice(1).join(":") || "" };
      return thumbnailUrl && thumbnailUrl !== item.thumbnailUrl ? { ...compatible, thumbnailUrl } : compatible;
    });
    return {
      ...
      stats: { total: all.length, current: current.length, archived: all.length - current.length, bilibili: current.filter((i) => i.platform === "bilibili").length, youtube: current.filter((i) => i.platform === "youtube").length, x: current.filter((i) => i.platform === "x").length }
    };
  }
  ...
  function assertCollectorSender(sender, platform) {
    ...
    const allowed = platform === "bilibili"
      ...
      : platform === "youtube"
        ? ["www.youtube.com", "youtube.com"].includes(url.hostname) && url.pathname === "/playlist" && url.searchParams.get("list") === "WL"
        : platform === "x" && url.hostname === "x.com" && (/^\/i\/(?:history|bookmarks)/.test(url.pathname) || /^\/[0-9A-Za-z_]{1,15}\/status\/\d+/.test(url.pathname));
    ...
    try {
      const response = await fetch(`${ai.baseUrl.replace(/\/$/, "")}/chat/completions`, {
        ...
        body: JSON.stringify({ model: ai.model, messages: [
          { role: "system", content: `你是内容分类器。主分类必须从以下列表选择：${rules.map((r) => r.name).join("、")}。只返回 JSON：{\"items\":[{\"id\":\"...\",\"primaryCategory\":\"...\",\"tags\":[最多5个],\"confidence\":0到1}]}` },
          { role: "user", content: JSON.stringify(items.map((item) => ({ id: item.id, title: item.title, text: clean(item.bodyText).slice(0, 500), creator: item.creator, creatorHandle: item.creatorHandle, platform: item.platform, nativeCategory: item.nativeCategory, durationSeconds: item.durationSeconds }))) }
        ] })
      });
