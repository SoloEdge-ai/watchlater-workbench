...
function createHarness({ failPut = false, platform = "youtube", existingTabs = [], onCreate, archived = false } = {}) {
  ...
  const account = isBilibili
    ...
    : isX
      ? { platform, id: "handle:@peng", name: "Peng", url: "https://x.com/peng" }
      : { platform, id: "handle:@peng", name: "Peng", url: "https://www.youtube.com/@peng" };
  const videoId = isBilibili ? "BV1test123" : isX ? "2091375708727857203" : "video123";
  ...
  const record = {
    id: recordId, platform, title: isX ? "Test post" : "Test video",
    sourceAccountId: account.id, sourceAccountName: account.name,
    status: archived ? "archived" : "current", manualArchived: archived
  };
  if (isX) Object.assign(record, { kind: "post", sourceItemId: videoId, url: `https://x.com/peng/status/${videoId}` });
  else record.videoId = videoId;
...
test("X restore opens the exact post and marks it current only after platform success", async () => {
  const harness = createHarness({ platform: "x", archived: true });
  const started = await harness.coordinator.start(harness.recordId, "restore");
  assert.equal(started.action.operation, "restore");
  assert.equal(started.action.sourceItemId, harness.videoId);
  assert.equal(harness.createdTabs[0].url, `https://x.com/peng/status/${harness.videoId}`);
  assert.equal(harness.records.get(harness.recordId).status, "archived");

  const claimed = await harness.coordinator.claim("x", harness.account, started.action.tabId);
  assert.equal(claimed.action.state, "restoring");
  await harness.coordinator.complete("x", claimed.action.id, started.action.tabId);

  const record = harness.records.get(harness.recordId);
  assert.equal(record.status, "current");
  assert.equal(record.manualArchived, false);
  assert.equal(record.sourceRestoreState, "complete");
});

test("X restore rejects a local URL that does not match the exact source post", async () => {
  const harness = createHarness({ platform: "x", archived: true });
  harness.records.get(harness.recordId).url = "https://x.com/peng/status/999999999";

  await assert.rejects(harness.coordinator.start(harness.recordId, "restore"), /原帖地址与帖子 ID 不一致/);
  assert.equal(harness.createdTabs.length, 0);
  assert.equal(harness.records.get(harness.recordId).status, "archived");
});

test("X restore recovery finishes the local write without clicking the platform again", async () => {
  const harness = createHarness({ platform: "x", archived: true });
  const started = await harness.coordinator.start(harness.recordId, "restore");
  const claimed = await harness.coordinator.claim("x", harness.account, started.action.tabId);
  harness.setFailPut(true);

  await assert.rejects(harness.coordinator.complete("x", claimed.action.id, started.action.tabId), /database unavailable/);
  assert.equal(harness.data.wlwPendingSourceAction_x.state, "platform_succeeded");
  assert.equal(harness.records.get(harness.recordId).status, "archived");

  harness.setFailPut(false);
  await harness.coordinator.reconcile();
  assert.equal(harness.records.get(harness.recordId).status, "current");
  assert.equal(harness.data.wlwPendingSourceAction_x, undefined);
});
