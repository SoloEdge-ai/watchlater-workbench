...
function renderSourceBindings() {
  ...
  for (const platform of ["bilibili", "youtube", "x"]) {
    const binding = sourceBindings[platform];
    const row = document.createElement("article"); row.className = "source-binding";
    const text = document.createElement("div");
    const title = document.createElement("b"); title.textContent = { bilibili: "B站", youtube: "YouTube", x: "X" }[platform];
...
async function exportAndClearBinding(platform) {
  ...
  const platformName = { bilibili: "B站", youtube: "YouTube", x: "X" }[platform] || platform;
  if (!confirm(`已导出 ${platformName} 数据。\n\n确认解除账号「${name}」的绑定吗？`)) return setStatus("已导出备份，未解除绑定");
  if (!confirm("再次确认：这会清除该平台在工作台中的内容、快照与操作状态；其他平台不受影响。")) return setStatus("已导出备份，未解除绑定");
