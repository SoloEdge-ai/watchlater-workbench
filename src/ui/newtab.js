...
const state = { settings: null, syncStatus: {}, sourceBindings: {}, sourceActionStatus: {}, pendingArchive: null, pendingRestore: null, query: { offset: 0, limit: PAGE_SIZE, platform: "all", contentType: "all", category: "all", tag: "all", rating: "", duration: "all", status: "current", sort: "priority", search: "" }, items: [], total: 0, nextOffset: null, facets: { categories: [], tags: [] }, selected: new Set() };
let searchTimer;
...
function bindEvents() {
  ...
  const mappings = { platformFilter: "platform", contentTypeFilter: "contentType", categoryFilter: "category", tagFilter: "tag", ratingFilter: "rating", durationFilter: "duration", statusFilter: "status", sortFilter: "sort" };
  ...
  document.getElementById("restoreLocalOnly").addEventListener("click", restoreLocalOnly);
  document.getElementById("restoreToSource").addEventListener("click", restoreToSource);
...
function renderStats(stats) {
  ...
  document.getElementById("xCount").textContent = stats.x;
...
function renderSyncStatus() {
  ...
  setSyncText("xSyncStatus", state.syncStatus.x);
}
...
function renderVideos(reset) {
  ...
  for (const item of (reset ? state.items : state.lastBatch)) grid.append(createContentCard(item));
...
function platformName(platform) {
  return { bilibili: "B站", youtube: "YouTube", x: "X" }[platform] || platform;
}

function createContentCard(item) {
  const isPost = item.kind === "post" || item.platform === "x";
  const card = el("article", `video-card${isPost ? " post-card" : ""}`);
  const visual = el("div", `thumb-wrap${isPost ? " post-visual" : ""}`);
  const visualLink = document.createElement("a");
  visualLink.className = "thumb-link";
  visualLink.href = item.url;
  visualLink.target = "_blank";
  visualLink.rel = "noopener noreferrer";
  visualLink.title = item.title;
  ...
  if (item.thumbnailUrl) {
    ...
    visual.classList.add("post-no-media");
    ...
    identity.append(textNode("b", "", item.creator || "X 用户"));
    identity.append(textNode("span", "", item.creatorHandle || ""));
    author.append(identity);
    visualLink.append(author, textNode("p", "post-copy", item.bodyText || item.title || "X 收藏"));
  } else {
    const image = document.createElement("img");
    image.alt = "";
    image.src = fallbackThumbnail(item.platform);
    visualLink.append(image);
  }

  visual.append(
    visualLink,
    textNode("span", "platform-badge", platformName(item.platform)),
    textNode("span", "score-badge", String(item.priorityScore ?? 50))
  );
  if (!isPost && item.durationSeconds) visual.append(textNode("span", "duration-badge", formatDuration(item.durationSeconds)));
  const checkbox = document.createElement("input");
  checkbox.type = "checkbox";
  checkbox.className = "select-video";
  ...
  const title = textNode("a", `card-title${isPost ? " post-text" : ""}`, item.bodyText || item.title || "无正文收藏");
  title.href = item.url;
  title.target = "_blank";
  title.rel = "noopener noreferrer";
  title.title = item.bodyText || item.title;
  const creator = textNode("p", "creator", [item.creator || "未知作者", item.creatorHandle].filter(Boolean).join(" · "));
  body.append(title, creator);
  if (isPost && item.quotedText) {
    body.append(textNode("p", "post-quote", `${item.quotedCreator ? `${item.quotedCreator}：` : "引用："}${item.quotedText}`));
  }

  const meta = el("div", "meta-row");
  meta.append(textNode("span", "chip primary", item.category || "待分类"));
  for (const tag of (item.tags || []).filter((tag) => tag !== item.category).slice(0, 2)) meta.append(textNode("span", "chip", tag));
  if (isPost && item.publishedAt) meta.append(textNode("span", "chip", new Date(item.publishedAt).toLocaleDateString("zh-CN")));
  const editTags = textNode("button", "chip", "＋标签");
  editTags.title = "编辑手动标签";
  editTags.addEventListener("click", async () => {
    const value = prompt("手动标签，用逗号分隔", (item.manualTags || []).join(", "));
  ...
  for (let rating = 1; rating <= 5; rating += 1) {
    const star = textNode("button", `star${Number(item.rating || 0) >= rating ? " active" : ""}`, "★");
    star.title = `${rating} 星想看程度`;
    star.addEventListener("click", async () => { await updateMeta(item.id, { rating: item.rating === rating ? null : rating }); });
    stars.append(star);
  }
  const category = document.createElement("select");
  category.className = "category-edit";
  category.append(new Option("自动分类", ""));
  for (const rule of state.settings.rules) category.append(new Option(rule.name, rule.name));
  category.value = item.manualCategory || "";
  category.addEventListener("change", () => updateMeta(item.id, { manualCategory: category.value }));
  ratingRow.append(stars, category);
  body.append(meta, ratingRow);

  const actionStatus = state.sourceActionStatus[item.platform];
  const isThisAction = actionStatus?.recordId === item.id;
  const operation = actionStatus?.operation || "remove";
  const localRecovery = isThisAction && actionStatus.state === SOURCE_ACTION_STATES.PLATFORM_SUCCEEDED;
  const actionFailure = isThisAction && actionStatus.state === SOURCE_ACTION_STATES.FAILED ? actionStatus.error : "";
  const storedFailure = item.sourceRestoreState === SOURCE_ACTION_STATES.FAILED
    ? item.sourceRestoreError
    : item.sourceRemovalState === SOURCE_ACTION_STATES.FAILED ? item.sourceRemovalError : "";
  const failure = actionFailure || storedFailure;
  if (localRecovery) body.append(textNode("p", "source-removal-error recovery", actionStatus.error || `${platformName(item.platform)} 已${operation === "restore" ? "重新收藏" : "移除"}，正在完成本地写入`));
  else if (failure) body.append(textNode("p", "source-removal-error", `平台操作失败：${failure}`));

  const buttonText = item.status === "archived" ? (item.platform === "x" ? "恢复收藏…" : "恢复到工作台") : failure ? "重试移出" : "移出工作台";
  const archiveButton = textNode("button", "archive-button", buttonText);
  archiveButton.addEventListener("click", async () => {
    if (item.status === "archived") {
      if (item.platform === "x") return openRestoreDialog(item);
      return updateMeta(item.id, { status: "current" });
    }
    openContentArchiveDialog(item);
  });
  ...
  if (!isPost) {
    ...
    fill.style.width = `${Math.min(100, item.durationSeconds ? (item.progressSeconds || 0) / item.durationSeconds * 100 : 0)}%`;
...
function openContentArchiveDialog(item) {
  ...
  document.getElementById("archiveVideoTitle").textContent = item.bodyText || item.title;
  sourceButton.textContent = binding
    ? `同时从 ${platformName(item.platform)} · ${binding.name || binding.id} 移除`
    : `同时从 ${platformName(item.platform)} 移除`;
  ...
  document.getElementById("archiveSourceHint").textContent = matched
    ? "平台页面会在可见标签页中打开；只有确认目标条目消失后，本地才会归档。"
    : "平台操作不可用：请先使用当前账号完成一次完整同步。";
...
async function archiveFromSource() {
  ...
  document.getElementById("archiveDialog").close();
  if (!result.ok) return toast(result.error, true);
  toast("已打开平台页面；验证账号后将只移除这一条内容");
  await loadLibrary(true);
}

function openRestoreDialog(item) {
  state.pendingRestore = item;
  const binding = state.sourceBindings[item.platform];
  const matched = Boolean(item.platform === "x" && binding && item.sourceAccountId && binding.id === item.sourceAccountId);
  const sourceButton = document.getElementById("restoreToSource");
  document.getElementById("restoreItemTitle").textContent = item.bodyText || item.title;
  sourceButton.textContent = binding ? `恢复并重新收藏到 X · ${binding.name || binding.id}` : "恢复并重新收藏到 X";
  sourceButton.disabled = !matched;
  document.getElementById("restoreSourceHint").textContent = matched
    ? "将打开这条 X 原帖；确认收藏按钮变为已收藏后，才会恢复工作台记录。"
    : "重新收藏不可用：请先使用当前 X 账号完成一次完整同步。";
  document.getElementById("restoreDialog").showModal();
}

async function restoreLocalOnly() {
  const item = state.pendingRestore;
  document.getElementById("restoreDialog").close();
  if (!item) return;
  await updateMeta(item.id, { status: "current" });
}

async function restoreToSource() {
  const item = state.pendingRestore;
  const button = document.getElementById("restoreToSource");
  if (!item || button.disabled) return;
  button.disabled = true;
  const result = await send({ type: "START_SOURCE_RESTORE", id: item.id });
  document.getElementById("restoreDialog").close();
  if (!result.ok) return toast(result.error, true);
  toast("已打开 X 原帖；确认重新收藏后将恢复工作台记录");
  await loadLibrary(true);
}
...
async function classifyWithAi() {
  ...
  if (!ids.length) return toast("请先勾选内容，或筛选出待分类项目");
  if (!confirm(`将向已配置的模型发送 ${ids.length} 条内容元数据，继续吗？`)) return;
  toast(`正在分类 ${ids.length} 条内容…`);
  const result = await send({ type: "AI_CLASSIFY", ids });
  if (!result.ok) return toast(result.error, true);
  toast(`AI 已更新 ${result.updated} 条内容`); await loadLibrary(true);
}
...
async function exportCsv() {
  ...
  const columns = ["platform", "kind", "contentType", "sourceItemId", "title", "bodyText", "creator", "creatorHandle", "quotedCreator", "quotedText", "publishedAt", "category", "tags", "rating", "priorityScore", "durationSeconds", "status", "url"];
...
function fallbackThumbnail(platform) { const names = { bilibili: "Bilibili", youtube: "YouTube", x: "X" }; const colors = { bilibili: "#fb7299", youtube: "#20242b", x: "#111111" }; return `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 360"><rect width="640" height="360" fill="${colors[platform] || "#20242b"}"/><text x="320" y="195" text-anchor="middle" font-size="56" fill="white" font-family="sans-serif">${names[platform] || "Content"}</text></svg>`)}`; }
