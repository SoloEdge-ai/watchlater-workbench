...
- 可选的单条平台删除同步：先严格验证已绑定账号，再通过可见平台页面移除。
...
首次完整同步会要求确认当前平台账号。B站使用 `mid`，YouTube 优先使用频道 ID、回退到唯一 handle；显示名称不作为身份依据。后续同步和删除都必须与绑定账号完全匹配。

点击视频卡的“移出工作台”后可选择：

- 仅移出工作台：只归档本地记录，不打开平台页面。
- 同时从平台移除：复用或打开可见的稍后再看页面，验证账号后按视频 ID 找到条目，再点击平台原生删除菜单。只有页面确认条目消失后，本地才归档。

平台删除只支持单条操作，不提供批量、自动删除或撤销。页面结构、菜单文案或账号不匹配时会安全失败并保留本地条目；同步写入前会逐批重新验证当前账号。浏览器或后台恢复后会重新驱动未完成操作；平台成功但本地写入暂时失败时，后台只补做本地归档，不会重复点击平台删除。

如需更换账号，在设置的“平台账号绑定”中选择“导出并解除绑定”。扩展会先下载该平台 JSON，再经两次确认清除该平台的视频、快照、操作状态和绑定；另一平台不受影响。
...
- `tabs`：根据明确的同步或单条删除操作打开、复用并聚焦可见平台页面。
- B站、YouTube Host 权限：采集稍后再看页面和读取 B站公开视频元数据。
- 可选 Host 权限：仅在启用自定义 AI 接口时按来源请求。

除用户在三选一对话框中明确选择的单条删除外，扩展不会自动、批量或通过后台请求修改平台播放列表。
