(function (root, factory) {
  ...
  async function run(adapter, deps = {}) {
    ...
    try {
      ...
      const operation = action.operation === "restore" ? "restore" : "remove";
      const target = action.title || action.sourceItemId || action.videoId;
      ensureOverlay().update(operation === "restore" ? `正在重新收藏：${target}` : `正在从账号中移除：${target}`);
      if (adapter.performAction) await adapter.performAction({ ...action, operation });
      else if (operation === "remove" && adapter.removeVideo) await adapter.removeVideo(action.videoId, { allowAlreadyMissing: action.allowAlreadyMissing === true });
      else throw new Error("当前页面 adapter 不支持该平台操作");
      ...
      if (!completed?.ok) {
        overlay.update(operation === "restore"
          ? `平台已重新收藏；本地恢复将在后台完成：${completed?.error || "写入失败"}`
          : `平台已移除；本地归档将在后台恢复：${completed?.error || "写入失败"}`, "error");
        return { completed: false, platformSucceeded: true, error: completed?.error };
      }
      ensureOverlay().update(operation === "restore" ? "平台重新收藏成功，已恢复到工作台" : "平台移除成功，本地已归档", "success");
      ...
      const restore = action?.operation === "restore";
      ensureOverlay().update(platformSucceeded
        ? restore ? `平台已重新收藏；本地恢复待完成：${text}` : `平台已移除；本地归档待恢复：${text}`
        : restore ? `重新收藏失败：${text}` : `移除失败：${text}`, "error");
