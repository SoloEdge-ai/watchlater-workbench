(function (root, factory) {
  ...
  function validateSourceSnapshot(snapshot) {
    ...
    if (Number.isInteger(expectedCount) && expectedCount >= 0) return validateCompleteSnapshot(snapshot.items, expectedCount);
    if (snapshot.complete !== true) return null;
    const unique = [...new Map(snapshot.items.filter((item) => item?.id).map((item) => [item.id, item])).values()];
    if (!unique.length && snapshot.allowEmpty !== true) return null;
    return unique;
  }

  return { BILIBILI_LINK_SELECTOR, clean, parseDuration, parseProgress, absoluteUrl, extractBilibiliVideoId, normalizeBilibiliCandidate, normalizeBilibiliApiResponse, normalizeYouTubeCandidate, validateCompleteSnapshot, validateSourceSnapshot };
});
